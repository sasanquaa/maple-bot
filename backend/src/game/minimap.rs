...
use super::{
    ...
    models::{Map, query_maps, upsert_map},
};
...
impl Contextual for Minimap {
    ...
    fn update(&self, context: &Context, mat: &Mat, state: &mut MinimapState) -> Self {
        match &context.minimap {
            Minimap::Detecting => {
                ...
                let Some((map, scale_w, scale_h)) = get_data_for_minimap(&bbox, &name) else {
                    return Minimap::Timeout(0);
                };
                state.data = map;
...
fn get_data_for_minimap(bbox: &Rect, name: &str) -> Option<(Map, f32, f32)> {
            ...
            match (bbox.width, bbox.height, map.width, map.height) {
                ...
                (b_w, b_h, m_w, m_h) if b_w < m_w && b_h < m_h => {
                    ...
                    map.actions.values_mut().flatten().for_each(|action| {
                        action.x = (action.x as f32 * w_ratio) as i32;
                        action.y = (action.y as f32 * h_ratio) as i32;
                    });
                    map.width = b_w;
                    map.height = b_h;
